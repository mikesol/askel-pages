  ...
  return (
    <div className="sticky top-16 z-40 bg-white border-b border-[#E9E4DF] shadow-sm">
      <div className="hidden md:flex max-w-5xl mx-auto px-6 h-12 items-center gap-1">
            ...
            {open === group.label && (
              <div className="absolute top-full left-0 bg-white rounded-xl shadow-lg border border-[#E9E4DF] py-2 min-w-[240px] z-50">
                {group.items.map((item) => (
                  ...
                  >
                    <span className="font-sora font-bold text-sm text-[#14375A] group-hover:text-[#FF8F7A] transition-colors">{item.label}</span>
        ...
        <a href={standalone.href} className="px-4 h-12 flex items-center font-sora font-bold text-xs text-[#14375A] hover:text-[#FF8F7A] transition-colors">
          {standalone.label}
        </a>
      </div>

      <div className="md:hidden">
        <button onClick={() => setMobileOpen(!mobileOpen)} className="w-full flex items-center justify-between px-6 h-12 font-sora font-bold text-xs text-[#14375A]">
        ...
        {mobileOpen && (
          <div className="border-t border-[#E9E4DF] pb-2">
            {groups.map((group) => (
              <div key={group.label}>
                <p className="font-sora font-bold text-xs text-[#14375A]/40 uppercase tracking-widest px-6 pt-4 pb-1">{group.label}</p>
                {group.items.map((item) => (
                  <a key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className="flex items-center justify-between px-6 py-2.5 hover:bg-[#F1F2F4]">
            ...
            <div className="border-t border-[#E9E4DF] mt-2">
              <a href={standalone.href} onClick={() => setMobileOpen(false)} className="flex items-center px-6 py-2.5 hover:bg-[#F1F2F4]">
