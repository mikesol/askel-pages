    ...
    >
      <div className="min-h-full flex items-start justify-center px-4 py-6">
        <div className="bg-white rounded-3xl w-full max-w-[680px] p-10 relative my-auto">
          ...
          <p className="font-sora text-[11px] font-bold tracking-widest text-[#FF8F7A] uppercase mb-2.5">Tilaa kuljetus</p>
          <h2 className="font-merriweather font-normal text-[#14375A] text-[32px] mb-2">Varaa nouto</h2>
          <p className="font-inter text-[#14375A]/65 text-[15px] mb-7">Valitse sinulle sopiva päivä ja aika.</p>

          {slotsError ? (
            <p className="font-inter text-red-500 text-sm">Ei saatavilla. Soita: +358 22 331718</p>
          ) : slotsLoading ? (
                  ...
                  return (
                    <button
                      ...
                      className={`border-2 rounded-2xl px-4 py-3 font-sora text-left transition-colors ${
                        isSelected ? 'border-[#FF8F7A] bg-[#FFF5F3]' : 'border-[#E9E4DF] bg-white hover:border-[#FF8F7A]'
                      }`}
                    >
          ...
          {step === 'form' && selectedSlot && (
            <div className="bg-[#F1F2F4] rounded-2xl p-7 max-w-[520px]">
              ...
              <div className="flex flex-col gap-3">
                <input type="text" placeholder="Nimi *" value={nimi} onChange={(e) => setNimi(e.target.value)} className="bg-white border border-[#E9E4DF] rounded-xl px-4 py-3 font-inter text-sm text-[#14375A] outline-none focus:border-[#FF8F7A] transition-colors" />
                <input type="text" placeholder="Nouto-osoite *" value={osoite} onChange={(e) => setOsoite(e.target.value)} className="bg-white border border-[#E9E4DF] rounded-xl px-4 py-3 font-inter text-sm text-[#14375A] outline-none focus:border-[#FF8F7A] transition-colors" />
                <input type="tel" placeholder="Puhelin" value={puhelin} onChange={(e) => setPuhelin(e.target.value)} className="bg-white border border-[#E9E4DF] rounded-xl px-4 py-3 font-inter text-sm text-[#14375A] outline-none focus:border-[#FF8F7A] transition-colors" />
                <input type="email" placeholder="Sähköposti" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-white border border-[#E9E4DF] rounded-xl px-4 py-3 font-inter text-sm text-[#14375A] outline-none focus:border-[#FF8F7A] transition-colors" />
                <textarea rows={3} placeholder="Lisätiedot (valinnainen)" value={lisatiedot} onChange={(e) => setLisatiedot(e.target.value)} className="bg-white border border-[#E9E4DF] rounded-xl px-4 py-3 font-inter text-sm text-[#14375A] outline-none focus:border-[#FF8F7A] transition-colors resize-none" />
                {formError && <p className="font-inter text-red-500 text-sm text-center">{formError}</p>}
                <button
          ...
          {step === 'success' && selectedSlot && (
            <div className="bg-[#14375A] text-white rounded-2xl p-8 text-center max-w-[520px]">
              ...
              <button onClick={handleClose} className="bg-[#FF8F7A] text-white font-sora font-bold text-sm px-6 py-3 rounded-full hover:bg-[#ff7a63] transition-colors">
                Sulje
              </button>
            </div>
          )}
          ...
          {step === 'error' && (
            <p className="font-inter text-red-500 text-sm mt-3">Virhe. Soita: +358 22 331718</p>
          )}
        </div>
      </div>
    </div>
